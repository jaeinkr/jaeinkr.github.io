...
### 읽어보며
---
딥러닝을 공부한지 몇년 이상 된 연구자의 입장에서, 비전 딥러닝의 기초지식을 다루는 Part 1에서는 크게 새로운 내용이 없어서 빠르게 훑고 넘어갔다.
이미지 분류와 탐지를 다루는 Part 2에서는 LeNet-5, AlexNet 등과 같은 유명한 비전 딥러닝 모델을 소개하고, 전이학습과 사물 탐지를 다룬다.
비전 딥러닝을 하고자 하는 이들이 꼭 공부해야 하는 내용들임을 알 수 있었다. 대부분 이름만 알거나 자세히 공부해보지 않은 내용임에도, 수월하게 이해할 수 있을 정도로 그림이 많고 설명이 친절했다.

NLP보다 CV의 생성 태스크가 더욱 많은 관심을 받는데, 생성 결과를 바로 직관적으로 볼 수 있어서 연구자들 뿐만 아니라 일반인들도 흥미로워해서 더욱 그런 듯 하다.
...
큰 감흥없이 Part 1를 훑고, '이 정도는 알아놓으면 좋지' 하는 의무감(?)으로 Part 2를 읽고, Part 3에서 생각지도 못하게 샘솟는 영감을 받게 되면서 말이다.
